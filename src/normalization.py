...
import pandas as pd
from .config import ACTIVE_STATUSES, PLANNED_STATUSES, TARGET_LANES, COMBO_TERMS, TODAY
...
def normalize_phase(phases: List[str]) -> str:
    ...
    phase_map = {"EARLY_PHASE1":"Early Phase 1","PHASE1":"Phase 1","PHASE2":"Phase 2","PHASE3":"Phase 3","PHASE4":"Phase 4","NA":"Phase not listed"}
    clean = [phase_map.get(str(p).strip().upper(), str(p).replace("_", " ").title()) for p in phases]
    joined = "/".join([c.strip() for c in clean if c.strip()])
    return joined.replace("Phase 1/Phase 2", "Phase 1/2").replace("Phase 2/Phase 3", "Phase 2/3") or "Phase not listed"
...
def extract_combo_category(row: pd.Series) -> str:
    ...
    if any(term.lower() in text for term in COMBO_TERMS):
        if any(term in text for term in ["pembrolizumab","nivolumab","atezolizumab","durvalumab","cemiplimab","pd-1","pd-l1","checkpoint","immunotherapy"]): return "Combination: IO / Checkpoint"
        if any(term in text for term in ["carboplatin","paclitaxel","gemcitabine","chemotherapy"]): return "Combination: Chemotherapy"
        if any(term in text for term in ["olaparib","niraparib","bevacizumab","targeted therapy"]): return "Combination: Targeted"
        return "Combination: Other"
    return "No named combo detected"


def extract_combo_agents(row: pd.Series) -> str:
    text = safe_str(row.get("text_blob", ""))
    agents = []
    for agent in ["pembrolizumab","nivolumab","atezolizumab","durvalumab","cemiplimab","carboplatin","paclitaxel","gemcitabine","bevacizumab","olaparib","niraparib"]:
        if re.search(agent, text, flags=re.IGNORECASE):
            agents.append(agent.title())
    return ", ".join(sorted(set(agents))) if agents else "No named combo agent detected"
